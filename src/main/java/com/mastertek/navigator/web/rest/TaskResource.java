...
import com.mastertek.navigator.web.rest.vm.CreateNewTaskVM;
...
public class TaskResource {
    ...
    @Timed
    public ResponseEntity<Task> createNewTask(@RequestBody CreateNewTaskVM createNewTaskVM) throws URISyntaxException {
        log.debug("REST request to save Task : {}", createNewTaskVM);
        
        Task task = new Task();
        task.setInsertDate(Instant.now());
        task.setVehicle(createNewTaskVM.getVehicle());
        task.setLat(createNewTaskVM.getLat());
        task.setLng(createNewTaskVM.getLng());
        
        String desc = createNewTaskVM.getIlce().getKey()+","+createNewTaskVM.getMahalle().getKey()+","
        	+createNewTaskVM.getSokak().getKey()+","+createNewTaskVM.getBina().getKey();
